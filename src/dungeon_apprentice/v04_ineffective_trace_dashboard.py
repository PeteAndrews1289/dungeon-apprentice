# ruff: noqa: E501
"""Read-only dashboard for the v0.4 matched ineffective-trace study."""
...
V04_DASHBOARD_HTML = """<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Dungeon Apprentice — v0.4</title>
...
</style></head><body><main><header><div><div class="eyebrow">v0.4 · matched architecture study</div>
<h1>Can it notice a bad habit forming?</h1></div><div class="fresh"><i class="dot"></i><span id="fresh">Loading…</span></div>
...
<span class="tiny">Scientific boundary</span><div class="note">Trace sham calibrates the comparison. Only the ineffective-trace
...
function arm(a){let p=Math.max(0,Math.min(100,100*Number(a.trained_actions||0)/1048576)),c=a.trace_encoder||{},
x=a.trace_summary||{},r=a.first_rollout||{},checks=(a.terminal_checks||[]).map((z,i)=>`<div class="check"><span>Terminal exam ${i+1}</span>
...
<span class="badge">${e(a.state)}</span></h2><div class="tiny">${a.trace_enabled?'Real bounded ineffective-action trace':'All-zero trace sham'}</div>
...
c.weight_l2_norm)}</b></div><div class="number"><span class="tiny">Trace active</span><b>${Number.isFinite(Number(x.trace_activation_rate))?
(100*Number(x.trace_activation_rate)).toFixed(1)+'%':'—'}</b></div><div class="number"><span class="tiny">First divergence</span><b>${e(
...
document.getElementById('decision').textContent=selected?'Ineffective-trace architecture selected':
...
let failures=0;async function poll(){try{let q=await fetch(`/api/v04.json?v=${Date.now()}`,{cache:'no-store'});
...
class V04DashboardError(RuntimeError):
    """Raised when dashboard evidence is unsafe or inconsistent."""


class V04DashboardServer(ThreadingHTTPServer):
    """HTTP server bound to one authenticated v0.4 cohort root."""

    daemon_threads = True
    allow_reuse_address = False
    run_root: Path
    snapshot_lock: threading.Lock
    authentication: V04DashboardAuthentication


@dataclass(frozen=True)
class V04DashboardAuthentication:
...
def _bounded_regular_bytes(
    ...
    *,
    ...
    try:
        ...
        raise V04DashboardError(f"{label} is missing or unsafe") from error
    try:
        ...
        if (
            ...
            raise V04DashboardError(f"{label} has an invalid file bound")
        chunks: list[bytes] = []
        remaining = metadata.st_size
        while remaining:
            chunk = os.read(descriptor, min(remaining, 1024 * 1024))
            if not chunk:
                raise V04DashboardError(f"{label} changed while read")
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise V04DashboardError(f"{label} grew while read")
...
def _absolute_directory(path: Path) -> Path:
    ...
    if not candidate.is_absolute():
        raise ValueError("v0.4 dashboard root must be absolute")
    try:
        metadata = candidate.lstat()
    except FileNotFoundError as error:
        raise ValueError("v0.4 dashboard root does not exist") from error
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
        raise ValueError("v0.4 dashboard root must be a regular directory")
...
def _read_json(
    ...
    *,
    ...
    try:
        ...
        raise V04DashboardError(f"dashboard evidence is missing: {path.name}") from error
    if (
        ...
        raise V04DashboardError(f"dashboard evidence is unsafe: {path.name}")
    try:
        payload = json.loads(path.read_bytes())
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V04DashboardError(f"cannot read dashboard evidence {path.name}") from error
    if not isinstance(payload, dict):
        raise V04DashboardError(f"dashboard evidence is not an object: {path.name}")
    return payload
...
def _evaluation_records(
    ...
    if ledger.exists() or ledger.is_symlink():
        ...
        if (
            ...
            raise V04DashboardError("v0.4 evaluations ledger is unsafe")
        ...
        try:
            for line in ledger.read_text(encoding="utf-8").splitlines():
                ...
                if not isinstance(payload, Mapping):
                    raise V04DashboardError("v0.4 evaluation row is not an object")
                rows.append(payload)
        except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise V04DashboardError("cannot read v0.4 evaluation ledger") from error
...
def _public_exam(exam: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        passed = all(v04.frozen_u2s.exam_stability_checks(exam).values())
...
def _safe_relative_frame(
    ...
    *,
    ...
    if arm_id not in ARM_ORDER:
        raise FileNotFoundError("unknown v0.4 arm")
    ...
    if not isinstance(relative_value, str):
        raise V04DashboardError("v0.4 frame path is invalid")
    relative = Path(relative_value)
    if relative.is_absolute() or ".." in relative.parts or relative.suffix.lower() != ".png":
        raise V04DashboardError("v0.4 frame path escapes its arm")
    arm_root = root / arm_id
    candidate = arm_root / relative
    if not candidate.exists() and not candidate.is_symlink():
        if allow_missing:
            return candidate
        raise FileNotFoundError("v0.4 frame is not yet available")
    metadata = candidate.lstat()
    if (
        stat.S_ISLNK(metadata.st_mode)
        or not stat.S_ISREG(metadata.st_mode)
        ...
        raise V04DashboardError("v0.4 frame is unsafe")
    resolved = candidate.resolve(strict=True)
    try:
        resolved.relative_to(arm_root.resolve(strict=True))
    except ValueError as error:
        raise V04DashboardError("v0.4 frame escapes its arm") from error
    with resolved.open("rb") as stream:
        if stream.read(8) != b"\x89PNG\r\n\x1a\n":
            raise V04DashboardError("v0.4 frame is not a PNG")
    if declared_sha256 is not None and (
        ...
        raise V04DashboardError("v0.4 frame checksum changed")
    return resolved


def resolve_v04_frame(run_root: Path, *, arm_id: str) -> Path:
...
def _authenticate_live_qualification(
    ...
    *,
    ...
    try:
        ...
        raise V04DashboardError("v0.4 qualification verifier is unavailable") from error
    ...
    try:
        ...
        raise V04DashboardError(
            f"live v0.4 qualification authentication failed: {error}"
        ) from error
    if not isinstance(public, dict):
        raise V04DashboardError("v0.4 qualification verifier returned no public evidence")
...
def _validate_contract(
    ...
    *,
    ...
    if (
        ...
        raise V04DashboardError("immutable v0.4 cohort contract is invalid")
    ...
    if live != dict(qualification):
        raise V04DashboardError("live v0.4 qualification binding changed")


def _startup_authentication(root: Path) -> V04DashboardAuthentication:
...
def _verify_cached_authentication(
    ...
    *,
    ...
    try:
        ...
        raise V04DashboardError("cached v0.4 qualification checksum is not ASCII") from error
    if checksum_fields != [
        digest,
        authentication.qualification_report.name,
    ]:
        raise V04DashboardError("cached v0.4 qualification checksum changed")
...
def _verified_process_closeout(
    ...
    *,
    ...
    if not isinstance(binding, Mapping):
        raise V04DashboardError("terminal process closeout evidence is missing")
    ...
    if (
        ...
        raise V04DashboardError("terminal process closeout evidence changed")
    seen: set[int] = set()
    ...
    for item in inventory:
        if (
            ...
            raise V04DashboardError("terminal process closeout inventory changed")
        seen.add(item["pid"])
    ...
    if [item["pid"] for item in inventory] != sorted(seen) or dashboards != expected_dashboards:
        raise V04DashboardError("terminal process closeout ordering changed")
    expected_binding = {
    ...
    if dict(binding) != expected_binding:
        raise V04DashboardError("terminal process closeout binding changed")
...
def _verified_closeout(
    ...
    *,
    ...
    if not isinstance(terminal, Mapping):
        raise V04DashboardError("v0.4 terminal state has no report binding")
    ...
    if (
        ...
        or terminal.get("stage_a_checkpoint_reuse_authorized") is not False
        or terminal.get("u3_authorized") is not False
        ...
        raise V04DashboardError("v0.4 terminal closeout is unauthenticated")
    ...
    for arm_id in ARM_ORDER:
        ...
        if not isinstance(exams, list) or not all(isinstance(exam, Mapping) for exam in exams):
            raise V04DashboardError(f"{arm_id} terminal exams are unavailable")
        arm_records[arm_id] = exams
    selection = v04.select_architecture(arm_records)
    if (
        report.get("selection") != selection
        or report.get("verdict") != selection["verdict"]
        or report.get("selected_architecture")
        != selection["selected_architecture"]
        or terminal.get("selected_architecture")
        != (
            "ineffective-trace"
            if selection["selected_architecture"] is not None
            else None
        )
    ):
        raise V04DashboardError("dashboard recomputation differs from v0.4 closeout")
    return report


def load_v04_snapshot(
    run_root: Path,
    *,
    authentication: V04DashboardAuthentication | None = None,
) -> dict[str, Any]:
    """Load one bounded public v0.4 view and reject provenance drift."""
    ...
    if (
        ...
        raise V04DashboardError("mutable v0.4 state differs from its contract")
    process_closeout = (
    ...
    for definition, arm_state in zip(contract["arms"], arms, strict=True):
        ...
        if directory.exists() or directory.is_symlink():
            if directory.is_symlink() or not directory.is_dir():
                raise V04DashboardError(f"unsafe v0.4 arm directory: {arm_id}")
            ...
            if (
                ...
                raise V04DashboardError(f"{arm_id} status differs from the cohort contract")
            ...
            raise V04DashboardError(f"{arm_id} state has no arm directory")
        if arm_state.get("state") == "completed":
            ...
            if not isinstance(raw_exams, list) or not all(
                ...
                raise V04DashboardError(f"{arm_id} report has no exam records")
            terminal = arm_state.get("terminal")
            if not isinstance(terminal, Mapping) or _sha256(
                directory / "report.json"
            ) != terminal.get("report_sha256"):
                raise V04DashboardError(f"{arm_id} terminal report changed")
            trace_encoder = report.get("trace_encoder")
            ...
            trace_encoder = status.get("encoder_latest", {})
            trace_summary = status.get("trace_metrics", {})
            first_rollout = {
                ...
                "first_divergence": (
                    ...
                    if isinstance(trace_encoder, Mapping)
                    and int(
                        trace_encoder.get(
        ...
        if arm_state.get("state") == "completed":
            try:
                gate_passed = v04.grade_terminal(
                    arm_id,
                    raw_exams,
                ).eligible
                eligible = arm_id == "ineffective-trace" and gate_passed
            except (RuntimeError, TypeError, ValueError) as error:
                raise V04DashboardError(f"{arm_id} terminal grade is invalid") from error
        ...
        public_arms.append(
            {
                ...
                "trace_enabled": definition.get("trace_enabled"),
                ...
                "eligible": eligible,
                "trace_encoder": (
                    dict(trace_encoder) if isinstance(trace_encoder, Mapping) else {}
                ),
                "trace_summary": (
                    dict(trace_summary) if isinstance(trace_summary, Mapping) else {}
                ),
                "first_rollout": (
...
class V04DashboardHandler(BaseHTTPRequestHandler):
    server: V04DashboardServer
    server_version = "DungeonApprenticeV04Dashboard/1"
    ...
    def _handle(self, *, send_body: bool) -> None:
        ...
        if parsed.path in {"/", "/index.html"}:
            self._send(
                V04_DASHBOARD_HTML.encode(),
                content_type="text/html; charset=utf-8",
                send_body=send_body,
            )
            return
        if parsed.path == "/api/v04.json":
            try:
                with self.server.snapshot_lock:
                    payload = load_v04_snapshot(
...
def start_v04_dashboard(
    ...
    *,
    ...
    load_v04_snapshot(root, authentication=authentication)
    server = V04DashboardServer(
        (host, int(port)),
        V04DashboardHandler,
    )
