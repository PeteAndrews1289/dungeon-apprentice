...
h2{font-size:18px;margin:0 0 14px}.tier{display:grid;grid-template-columns:110px 1fr 125px;gap:12px;
...
border-radius:999px;padding:6px 10px;color:var(--muted);font-size:12px}.funnel{color:var(--muted);margin-top:12px}.examFrames{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:18px}.examShot{margin:0}.examShot img{width:100%;aspect-ratio:1;object-fit:contain;image-rendering:pixelated;background:#050806;border-radius:8px;border:1px solid var(--line)}.examShot figcaption{color:var(--muted);font-size:11px;margin-top:4px;overflow-wrap:anywhere}.history{overflow-x:auto}.events{width:100%;min-width:620px;border-collapse:collapse}.events td,.events th{padding:9px 8px;
...
<div id="examFrames" class="examFrames"></div>
...
function render(d){let warm=Boolean(d.warm_start),collected=Number(warm?(d.child_collected_timesteps??0):(d.collected_timesteps??d.total_timesteps??0)),trained=Number(warm?(d.child_trained_timesteps??0):(d.trained_timesteps??0)),waiting=Math.max(0,collected-trained);
byId('collected').textContent=fmt(collected);byId('trained').textContent=fmt(trained);byId('pending').textContent=waiting?`${fmt(waiting)} steps await the next learning update.`:warm?`New U1 experience only; the inherited parent had ${fmt(Number(d.trained_timesteps||0)-trained)} learned steps.`:'All collected experience has been learned from.';
...
byId('tiers').innerHTML=latest.length?latest.map(x=>`<div class="tier"><strong>${x.lesson_label||x.tier_label||'Lesson'}</strong><div class="bar"><div class="fill" style="width:${pct(x.success_rate)}%"></div></div><div class="value">${fmt(x.successes)}/${fmt(x.episodes)} · ${pct(x.success_rate)}%</div></div>`).join(''):'<div class="empty">No exam completed yet.</div>';
...
let allocation=d.last_completed_practice_allocation||d.practice_allocation||{},targets=allocation.target_shares||{},realized=allocation.realized_shares||{},pills=Object.keys(targets).map(k=>`<span class="pill">${k.split('/').pop()}: target ${pct(targets[k])}% · actual ${pct(realized[k])}%</span>`);if(warm)pills.unshift(`<span class="pill">Warm-start child · parent ${String(d.parent_checkpoint_sha256||'').slice(0,8)}</span>`);byId('practice').innerHTML=pills.join('');
let frames=d.lesson_frames||{};byId('examFrames').innerHTML=Object.entries(frames).map(([lesson,path])=>`<figure class="examShot"><img src="${path}?v=${d.frame_revision||0}" alt="Latest frozen exam frame for ${lesson}"><figcaption>${lesson}</figcaption></figure>`).join('');
let recovery=d.curriculum&&d.curriculum.recovery,cause=d.curriculum&&d.curriculum.recovery_cause;byId('gateNote').textContent=d.protocol==='dungeon-apprentice-v0.2-u1'?`Two cumulative U1 passes are required. Navigate, Visible Unlock, and Local Unlock are all examined.${recovery?` Recovery practice is targeting ${cause}.`:''}`:d.protocol==='dungeon-apprentice-v0.2-sentinel'?`Two passing exams are required. Navigate retention: 85%.${recovery?' Recovery practice is active.':''}`:'90% passes the current lesson. Earlier lessons must remain at or above 80%.';
let hist=Array.isArray(d.evaluation_history)?d.evaluation_history.slice().reverse():[];byId('history').innerHTML=hist.length?`<table class="events"><thead><tr><th>New learned step</th><th>Lesson</th><th>Success</th><th>Decision</th></tr></thead><tbody>${hist.map(x=>`<tr><td>${fmt(x.child_trained_timesteps??x.trained_timesteps)}</td><td>${x.tier_label||'Lesson'}</td><td>${pct(x.success_rate)}%</td><td>${x.decision||'Measured'}</td></tr>`).join('')}</tbody></table>`:'<div class="empty">The first exam has not run yet.</div>'}
