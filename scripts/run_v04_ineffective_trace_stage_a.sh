...
qualification_report="$dungeon_root/qualifications/v0.4-ineffective-trace-stage-a-20260724/report.json"
...
qualifier_module="dungeon_apprentice.v04_ineffective_trace_qualify"
...
if (( $# != 0 )); then
  print -u2 "the fixed v0.4 Stage-A launcher accepts no options; interruption makes both fresh twins terminal"
  exit 2
fi
...
authenticate_published_training_tag() {
  ...
  object_type=$(git cat-file -t "refs/tags/$training_tag" 2>/dev/null) || {
    print -u2 "required v0.4 annotated tag is missing: $training_tag"
    return 1
  }
  [[ "$object_type" == tag ]] || {
    print -u2 "v0.4 source tag must be an annotated tag object"
    return 1
  }
  ...
  [[ "$peeled" == "$source_commit" ]] || {
    print -u2 "v0.4 tag does not peel to the clean source commit"
    return 1
  }
  remote_url=$(git remote get-url origin)
  [[ "$remote_url" == "$expected_origin" ]] || {
    print -u2 "v0.4 origin differs from its preregistered repository"
    return 1
  }
  remote_line=$(git ls-remote --tags origin "refs/tags/$training_tag")
  [[ "$remote_line" == "$tag_object"$'\t'"refs/tags/$training_tag" ]] || {
    print -u2 "v0.4 annotated tag is not published unchanged on origin"
    return 1
  }
}
...
assert_source_unchanged() {
  ...
  measured=$(clean_source_commit) || {
    print -u2 "v0.4 source became dirty or lost its committed HEAD"
    return 1
  }
  [[ "$measured" == "$source_commit" ]] || {
    print -u2 "v0.4 source commit changed after authentication"
    return 1
  }
  ...
  [[ "$measured_tag" == "$tag_object" && "$peeled" == "$source_commit" ]] || {
    print -u2 "v0.4 annotated tag identity changed"
    return 1
  }
}
...
if free < required:
    raise SystemExit(
        f"v0.4 Stage A requires {minimum_gib} GiB free, but only "
...
assert_regular_ancestor_chain() {
  ...
  while true; do
    if [[ ! -d "$current" || -L "$current" ]]; then
      print -u2 "v0.4 artifact path has a missing or symlinked ancestor: $current"
      return 1
    fi
    [[ "$current" == "$stop" ]] && return 0
    if [[ "$current" == "/" ]]; then
      print -u2 "v0.4 artifact path escapes its fixed storage root"
      return 1
    fi
    current=${current:h}
  done
...
if (
    ...
    or Path("/Volumes/T7 Developer/DungeonApprentice/v04-ineffective-trace-stage-a-20260724")
    != CANONICAL_COHORT_ROOT
    or Path("/Volumes/T7 Developer/DungeonApprentice/v04-ineffective-trace-stage-a-media-20260724")
    != CANONICAL_MEDIA_ROOT
    or QUALIFIED_TAG != "ineffective-trace-architecture-v0.4-stage-a-20260724"
):
    raise SystemExit("v0.4 canonical identities differ from the launcher")
evidence = verify_ineffective_trace_qualification(
    report_path,
...
if (
    ...
    or not public.get("r3_terminal_evidence_sha256")
):
    raise SystemExit("v0.4 qualification identity differs from this launch")
PY
}
  ...
  /bin/ps -axo pid=,command= | /usr/bin/awk '
    ...
    function is_trainer_module(name) {
      return name == "dungeon_apprentice.train" \
        ...
        || name == "dungeon_apprentice.v04_ineffective_trace_train"
    }
    function is_trainer_executable(name) {
      return name == "dungeon-train" \
        || name == "dungeon-train-v02-sentinel" \
        || name == "dungeon-train-v02-u1" \
        || name == "dungeon-train-v02-u2" \
        || name == "dungeon-train-v02-u2r" \
        || name == "dungeon-train-v02-u2s" \
        || name == "dungeon-train-v03-action-effect" \
        || name == "dungeon-train-v04-ineffective-trace"
    }
    {
      pid = $1
      executable = basename($2)
      first_argument = basename($3)
      if (is_trainer_executable(executable)) {
        print pid
      } else if (is_python(executable)) {
        if (is_trainer_executable(first_argument)) {
          print pid
...
if (
    ...
    raise SystemExit("dashboard identity differs from this v0.4 cohort")
PY
}
...
start_dashboard() {
  ...
  if [[ -n "$listener" ]]; then
    print -u2 "v0.4 dashboard port $dashboard_port is already in use"
    return 1
  fi
...
PY
  ...
  for _attempt in {1..300}; do
    ...
    if assert_dashboard_healthy 2>/dev/null; then
      ...
      print "v0.4 Stage-A dashboard: http://127.0.0.1:$dashboard_port/"
      return 0
    fi
    /bin/sleep 0.1
  done
  kill "$dashboard_pid" 2>/dev/null || true
  print -u2 "v0.4 dashboard did not become healthy on /api/v04.json"
  return 1
}
...
if phase not in {"completed", "interrupted", "crashed"}:
    raise SystemExit(f"unexpected v0.4 arm phase: {phase!r}")
print(phase)
PY
}
...
if (
    ...
    raise SystemExit("v0.4 supervisor identity is invalid")
...
capture_owned_supervisor_identity() {
  ...
  print -u2 "v0.4 could not capture its owned supervisor process identity"
  return 1
}
...
wait_for_supervisor_exit() {
  ...
  for (( _poll = 0; _poll < polls; _poll++ )); do
    ...
    if [[ "$measured" != "$supervisor_identity" ]]; then
      print -u2 \
        "v0.4 refused to wait on a supervisor PID with a changed identity"
      return 2
    fi
...
signal_owned_supervisor() {
  ...
  if ! owned_supervisor_is_live; then
    print -u2 \
      "v0.4 refused to signal a supervisor without its captured identity"
    return 1
  fi
...
kill_owned_trainer_group() {
  ...
  capture_owned_trainer_identity || {
    print -u2 \
      "v0.4 refused to kill a trainer group without authenticated supervisor evidence"
    return 1
  }
  ...
  [[ "$measured" == "$trainer_identity" \
    && "$parent" == "$supervisor_pid" \
    && "$group" == "$trainer_pid" ]] || {
    print -u2 "v0.4 refused to kill a reused or unowned trainer process group"
    return 1
  }
...
shutdown_supervised_chain() {
  ...
  if ! owned_supervisor_is_live; then
    ...
    print -u2 "v0.4 cannot authenticate its supervisor for shutdown"
    return 1
  fi
  ...
  print -u2 "v0.4 trainer ignored graceful stop; escalating through supervisor"
  signal_owned_supervisor TERM || return 1
  ...
  print -u2 "v0.4 trainer ignored TERM; killing its authenticated process group"
  ...
  print -u2 "v0.4 supervisor did not reap the killed trainer; forcing supervisor exit"
...
assert_single_active_chain() {
  ...
  [[ "$trainers" == "$trainer_pid" ]] || {
    print -u2 "v0.4 does not have exactly one neural trainer"
    return 1
  }
  [[ "$supervisors" == "$supervisor_pid" ]] || {
    print -u2 "v0.4 does not have exactly one trainer supervisor"
    return 1
  }
  [[ "$caffeinates" == "$caffeine_pid" ]] || {
    print -u2 "v0.4 does not have exactly one caffeinate process"
    return 1
  }
  trainer_identity=$(process_identity "$trainer_pid") || {
    print -u2 "v0.4 could not capture its trainer process identity"
    return 1
  }
}
...
if plan.get("done") is True and plan.get("closeout_required") is True:
    print("closeout")
elif plan.get("done") is False and plan.get("arm") in {"trace-sham", "ineffective-trace"}:
...
PY
  ...
  if [[ "$disposition" == closeout ]]; then
    # Both arms are complete, but an abnormal launcher exit invalidates this
    # non-resumable matched root even if a process snapshot was already
    # written. Preserve every artifact and durably close the finalization path.
    manifest_command abort-closeout \
      --root "$run_root" \
      --source-commit "$source_commit" \
      --tag-object "$tag_object" \
      --launcher-exit-code "$exit_code" >/dev/null 2>&1 || true
    return 0
  fi
...
request_launcher_stop() {
  ...
  abort_launcher 130 \
    "v0.4 launcher stop requested; closing the supervised chain"
}
...
require_launcher_step \
  "v0.4 could not authenticate the mounted storage volume" \
  assert_actual_mounted_volume
...
require_launcher_step \
  "v0.4 storage root failed its regular-ancestor check" \
...
for helper in "$manifest_helper" "$trainer_supervisor"; do
  if [[ ! -f "$helper" || -L "$helper" ]]; then
    abort_launcher 1 "v0.4 launcher helper is missing or unsafe: $helper"
  fi
done
if ! "$repository/.venv/bin/python" -c \
  "import $trainer_module, $dashboard_module, $qualifier_module" >/dev/null 2>&1; then
  abort_launcher 1 "project environment cannot import the fixed v0.4 runtime"
fi

require_launcher_step \
  "v0.4 could not authenticate the frozen parent checkpoint" \
  assert_frozen_parent
source_commit=$(clean_source_commit) || {
  abort_launcher 1 \
    "refusing to launch v0.4 Stage A from dirty or uncommitted source"
}
require_launcher_step \
  "v0.4 could not authenticate its published training tag" \
  authenticate_published_training_tag
require_launcher_step \
  "v0.4 qualification authentication failed" \
  assert_v04_qualification
if [[ -e "$run_root" || -L "$run_root" || -e "$media_root" || -L "$media_root" ]]; then
  abort_launcher 1 \
    "refusing to reuse the canonical fresh v0.4 Stage-A roots"
fi
require_launcher_step "v0.4 free-space preflight failed" assert_free_space
require_launcher_step \
  "v0.4 found a competing training chain during preflight" \
  assert_no_competing_training_chain
[[ -z "$(dashboard_listener_pid)" ]] || {
  abort_launcher 1 "v0.4 dashboard port $dashboard_port is already in use"
}

require_launcher_step \
  "v0.4 could not create its canonical cohort root" \
  /bin/mkdir -m 0755 "$run_root"
require_launcher_step \
  "v0.4 could not create its canonical media root" \
  /bin/mkdir -m 0755 "$media_root"
require_launcher_step \
  "v0.4 cohort root failed its regular-ancestor check" \
  assert_regular_ancestor_chain "$run_root/placeholder" "$dungeon_root"
require_launcher_step \
  "v0.4 media root failed its regular-ancestor check" \
  assert_regular_ancestor_chain "$media_root/placeholder" "$dungeon_root"
require_launcher_step \
  "v0.4 could not create its cohort manifest" \
...
require_launcher_step \
  "v0.4 source changed after cohort creation" \
  assert_source_unchanged
require_launcher_step "v0.4 free-space check failed after cohort creation" \
  assert_free_space
require_launcher_step \
  "v0.4 found a competing chain after cohort creation" \
  assert_no_competing_training_chain
require_launcher_step "v0.4 dashboard startup failed" start_dashboard

/usr/bin/caffeinate -ims >/dev/null 2>&1 &
caffeine_pid=$!
require_launcher_step \
  "v0.4 caffeinate process did not remain alive" \
  kill -0 "$caffeine_pid"

while true; do
  require_launcher_step \
    "v0.4 source changed before the next arm" \
    assert_source_unchanged
  require_launcher_step \
    "v0.4 free-space check failed before the next arm" \
    assert_free_space
  require_launcher_step \
    "v0.4 dashboard health failed before the next arm" \
    assert_dashboard_healthy
  [[ -z "$(active_neural_trainers)" ]] || {
    abort_launcher 1 "a neural trainer exists before the next v0.4 arm"
  }
  [[ -z "$(active_trainer_supervisors)" ]] || {
    abort_launcher 1 \
      "a trainer supervisor exists before the next v0.4 arm"
  }
  [[ "$(active_caffeinates)" == "$caffeine_pid" ]] || {
    abort_launcher 1 \
      "the v0.4 launcher lost its sole caffeinate identity"
  }
  plan_json=$(
    manifest_command next \
      --root "$run_root" \
      --source-commit "$source_commit" \
      --tag-object "$tag_object"
  ) || abort_launcher 1 "v0.4 next-arm planning failed"
  [[ -n "$plan_json" ]] || {
    abort_launcher 1 "v0.4 next-arm planner returned no JSON"
  }
  plan_fields=("${(@f)$(
    "$repository/.venv/bin/python" - "$plan_json" <<'PY'
import json
...
if plan.get("done") is True:
    ...
    for key in ("arm", "attempt", "run_dir", "media_dir"):
        print(plan[key])
PY
  )}") || abort_launcher 1 "v0.4 next-arm plan parsing failed"
  if [[ "${plan_fields[1]}" == done ]]; then
    if (( ${#plan_fields} != 2 )) || [[ "${plan_fields[2]}" != true ]]; then
      abort_launcher 1 \
        "v0.4 planner reached done without a required closeout"
    fi
    break
  fi
  if (( ${#plan_fields} != 5 )) || [[ "${plan_fields[1]}" != run ]]; then
    abort_launcher 1 \
      "v0.4 next-arm planner returned an incomplete exact plan"
  fi
  active_arm=${plan_fields[2]}
  attempt=${plan_fields[3]}
  run_directory=${plan_fields[4]}
  media_directory=${plan_fields[5]}
  if [[ "$active_arm" != trace-sham && "$active_arm" != ineffective-trace ]] \
    || [[ "$attempt" != 0 ]] \
    || [[ "$run_directory" != "$run_root/$active_arm" ]] \
    || [[ "$media_directory" != "$media_root/$active_arm" ]]; then
    abort_launcher 1 "v0.4 planner changed its fixed arm identity"
  fi

  require_launcher_step \
    "v0.4 could not claim the next arm in its manifest" \
  ...
  if [[ -e "$run_directory" || -L "$run_directory" ]] \
    ...
    abort_launcher 1 "refusing to reuse a fresh v0.4 arm target"
  fi
  require_launcher_step \
    "v0.4 could not create its fresh arm run directory" \
    /bin/mkdir -m 0755 "$run_directory"
  require_launcher_step \
    "v0.4 could not create its fresh arm media directory" \
    /bin/mkdir -m 0755 "$media_directory"
  require_launcher_step \
    "v0.4 arm run directory failed its regular-ancestor check" \
    assert_regular_ancestor_chain "$run_directory/placeholder" "$dungeon_root"
  require_launcher_step \
    "v0.4 arm media directory failed its regular-ancestor check" \
  ...
  if [[ -e "$supervisor_state" || -L "$supervisor_state" ]]; then
    abort_launcher 1 \
      "refusing to replace v0.4 supervisor evidence: $supervisor_state"
  fi
  print "Starting v0.4 Stage-A arm $active_arm (fresh matched twin)."
  ...
  trainer_identity=""
  require_launcher_step \
    "v0.4 could not authenticate its newly launched supervisor" \
    capture_owned_supervisor_identity
  ...
  if [[ "$status_ready" == true ]] && kill -0 "$supervisor_pid" 2>/dev/null; then
    require_launcher_step \
      "v0.4 active trainer chain identity check failed" \
  ...
  if [[ "$status_ready" != true || ! -f "$status_path" || -L "$status_path" ]]; then
    require_launcher_step \
      "v0.4 could not record its pre-status trainer crash" \
      manifest_command finish-arm \
    ...
    active_arm=""
    abort_launcher 1 \
      "v0.4 arm stopped before publishing safe status evidence"
  fi
  terminal_phase=$(read_status_phase "$status_path") \
    || abort_launcher 1 "v0.4 terminal arm status authentication failed"
  require_launcher_step \
    "v0.4 source changed after an arm exited" \
    assert_source_unchanged
  require_launcher_step \
    "v0.4 free-space check failed after an arm exited" \
    assert_free_space
  require_launcher_step \
    "v0.4 dashboard health failed after an arm exited" \
    assert_dashboard_healthy
  if [[ "$terminal_phase" == interrupted ]]; then
    require_launcher_step \
      "v0.4 could not record its interrupted arm outcome" \
      manifest_command finish-arm \
        --root "$run_root" --source-commit "$source_commit" \
    ...
    active_arm=""
    abort_launcher 130 \
      "v0.4 arm was interrupted; the complete matched root is terminal operationally_incomplete."
  fi
  if [[ "$terminal_phase" == crashed || "$trainer_status" -ne 0 ]]; then
    require_launcher_step \
      "v0.4 could not record its crashed arm outcome" \
      manifest_command finish-arm \
        --root "$run_root" --source-commit "$source_commit" \
        --tag-object "$tag_object" --arm "$active_arm" --outcome crashed \
        --trainer-exit-code "$trainer_status" >/dev/null
    active_arm=""
    abort_launcher 1 \
      "v0.4 arm failed; this root can never be resumed or reused."
  fi
  require_launcher_step \
    "v0.4 could not record its completed arm outcome" \
    manifest_command finish-arm \
      --root "$run_root" \
      --source-commit "$source_commit" \
      --tag-object "$tag_object" \
      ...
      --outcome completed \
      --trainer-exit-code "$trainer_status" >/dev/null
  print "Completed v0.4 arm $active_arm at exactly 1,048,576 child actions."
  active_arm=""
done

require_launcher_step \
  "v0.4 source changed before cohort closeout" \
  assert_source_unchanged
require_launcher_step \
  "v0.4 free-space check failed before cohort closeout" \
  assert_free_space
[[ -z "$(active_neural_trainers)" ]] || {
  abort_launcher 1 "a neural trainer remained after both v0.4 arms"
}
[[ -z "$(active_trainer_supervisors)" ]] || {
  abort_launcher 1 "a trainer supervisor remained after both v0.4 arms"
}
require_launcher_step \
  "v0.4 dashboard health failed before process closeout" \
  assert_dashboard_healthy
stop_caffeine
require_launcher_step \
  "v0.4 could not seal its process closeout" \
  manifest_command seal-process-closeout \
    --root "$run_root" \
    --source-commit "$source_commit" \
    --tag-object "$tag_object" >/dev/null
require_launcher_step \
  "v0.4 could not finalize its matched cohort" \
  manifest_command finalize \
    --root "$run_root" \
    --source-commit "$source_commit" \
    --tag-object "$tag_object" >/dev/null
require_launcher_step \
  "v0.4 finalized dashboard health failed" \
  assert_dashboard_healthy
launcher_finalized=true
trap - EXIT INT TERM
print "v0.4 Stage A completed and sealed its matched architecture decision."
