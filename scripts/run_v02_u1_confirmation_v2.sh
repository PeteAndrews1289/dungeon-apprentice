...
output="$volume/DungeonApprentice/confirmations/v0.2-u1-v2-20260723/report.json"
...
prior_attempt_report="$volume/DungeonApprentice/confirmations/v0.2-u1-20260723/report.json"
prior_attempt_sha256="d2fa53308f7488cc08f5ee67b86a125ad91c6ba3790fcd9433c35aab1215b25c"
checkpoints=(
...
payload = {
    "schema_version": 1,
    "protocol": "dungeon-apprentice-v0.2-u1-confirmation-v2-attempt",
    "status": "started",
    "started_at": now,
    "completed_at": None,
    ...
    "prior_attempt_report": prior_attempt_report,
...
if [[ ! -x .venv/bin/dungeon-confirm-v02-u1-v2 ]]; then
  print -u2 "project environment does not contain the U1 confirmation v2 command"
  exit 1
fi

if [[ -n "$(git status --porcelain)" ]]; then
  print -u2 "refusing to open the v2 confirmation candidate streams from a dirty working tree"
  exit 1
fi
...
if [[ ! -f "$prior_attempt_report" ]]; then
  print -u2 "immutable v1 confirmation attempt report is missing: $prior_attempt_report"
  exit 1
fi

prior_attempt_checksum_line=$(/usr/bin/shasum -a 256 "$prior_attempt_report")
measured_prior_attempt_sha256=${prior_attempt_checksum_line%% *}
if [[ "$measured_prior_attempt_sha256" != "$prior_attempt_sha256" ]]; then
  print -u2 "immutable v1 confirmation attempt report digest does not match the frozen SHA-256"
  exit 1
fi
...
if .venv/bin/dungeon-confirm-v02-u1-v2 \
    --checkpoint "${checkpoints[1]}" \
    --checkpoint "${checkpoints[2]}" \
    --checkpoint "${checkpoints[3]}" \
    --prior-attempt-report "$prior_attempt_report" \
